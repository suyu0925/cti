...
BOOL CXMSApi_PlaySet::OnInitDialog() 
{
	...
	if ( g_PlayProperty.m_u8SrcMode == XMS_SRC_11K )
		((CButton *)GetDlgItem ( IDC_RADIO_SRC_11K ))->SetCheck(1);
	else
		((CButton *)GetDlgItem ( IDC_RADIO_SRC_11K ))->SetCheck(0);
	...
	sprintf ( TmpStr, "%c", g_PlayProperty.m_u32StopCode );
...
void CXMSApi_PlaySet::OnOK() 
{
	...
	if ( ((CButton *)GetDlgItem ( IDC_RADIO_SRC_11K ))->GetCheck() )
		g_PlayProperty.m_u8SrcMode = XMS_SRC_11K;
    ...
    g_PlayProperty.m_u32StopCode = TmpStr[0];
	...
	g_PlayProperty.m_u16FileOffset = iTmpVal/8000;
    g_PlayProperty.m_u16FileSubOffset = iTmpVal%8000;
	//playpropetry . m_u16PlayType
...
void CXMSApi_PlaySet::Enable_File(bool IsEnable)
{
	...
	GetDlgItem ( IDC_RADIO_SRC_11K )->EnableWindow(IsEnable);
