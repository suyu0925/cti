...
DJ_U16  cfg_bargeInEnable = 0;   // bargeIn enable

DJ_U16  cfg_bargeDTMFCutOff = 1; // 1: DTMF and tone cut off, will be not as voc

DJ_U32  cfg_bargePersonSilenceTime = 700; //真人摘机的最大静音时长，

DJ_U32   cfg_bargeSilenceTout= 12000; //静音超时时长

DJ_U32  cfg_bargeMusicOnTime = 800; //彩铃的最大语音时长, 

DJ_U32   cfg_bargeVocTout = 11000; //语音超时时长

DJ_U32  cfg_bargeToneSilenceTime= 200; //提示音的最大静音时长,

DJ_U16   cfg_bargeGetNormEvt=0; //  1 : 获取noraml事件，0:不获取noraml事件
...
void	ReadFromConfig(void)
{
	...
	//add by zcq at 20121015, for  AMD detest

	cfg_bargeInEnable= GetPrivateProfileInt ( "ConfigInfo", "bargeInEnable", 0, cfg_IniName);
       cfg_bargeDTMFCutOff = GetPrivateProfileInt ( "ConfigInfo", "bargeDTMFCutOff", 1, cfg_IniName);
       cfg_bargePersonSilenceTime = GetPrivateProfileInt ( "ConfigInfo", "bargePersonSilenceTime", 700, cfg_IniName);
       cfg_bargeSilenceTout = GetPrivateProfileInt ( "ConfigInfo", "bargeSilenceTout", 12000, cfg_IniName);
       cfg_bargeMusicOnTime = GetPrivateProfileInt ( "ConfigInfo", "bargeMusicOnTime", 800, cfg_IniName);
       cfg_bargeVocTout = GetPrivateProfileInt ( "ConfigInfo", "bargeVocTout", 11000, cfg_IniName);

       cfg_bargeToneSilenceTime = GetPrivateProfileInt ( "ConfigInfo", "bargeToneSilenceTime", 200, cfg_IniName);

       cfg_bargeGetNormEvt = GetPrivateProfileInt ( "ConfigInfo", "bargeGetNormEvt", 0, cfg_IniName);
...
void	WriteToConfig(void)
{
	...
	//add by zcq at 20121015, for  AMD detest
	sprintf ( TmpStr, "%d", cfg_bargeInEnable);
	WritePrivateProfileString ( "ConfigInfo", "bargeInEnable", TmpStr, cfg_IniName);

	sprintf ( TmpStr, "%d", cfg_bargeDTMFCutOff);
	WritePrivateProfileString ( "ConfigInfo", "bargeDTMFCutOff", TmpStr, cfg_IniName);

	sprintf ( TmpStr, "%d", cfg_bargePersonSilenceTime);
	WritePrivateProfileString ( "ConfigInfo", "bargePersonSilenceTime", TmpStr, cfg_IniName);


	sprintf ( TmpStr, "%d", cfg_bargeSilenceTout);
	WritePrivateProfileString ( "ConfigInfo", "bargeSilenceTout", TmpStr, cfg_IniName);

	sprintf ( TmpStr, "%d", cfg_bargeMusicOnTime);
	WritePrivateProfileString ( "ConfigInfo", "bargeMusicOnTime", TmpStr, cfg_IniName);

	sprintf ( TmpStr, "%d", cfg_bargeVocTout);
	WritePrivateProfileString ( "ConfigInfo", "bargeVocTout", TmpStr, cfg_IniName);

	sprintf ( TmpStr, "%d", cfg_bargeToneSilenceTime);
	WritePrivateProfileString ( "ConfigInfo", "bargeToneSilenceTime", TmpStr, cfg_IniName);

	sprintf ( TmpStr, "%d", cfg_bargeGetNormEvt);
	WritePrivateProfileString ( "ConfigInfo", "bargeGetNormEvt", TmpStr, cfg_IniName);
...
void openMsgLogFile()
{
	...
	char   LogFileShortName[] = "\\dbugLog.txt";
	char   MsgStr[512] = {0};
...
// baregeIn and AMD func---zcq at 20120829
DJ_S32 startBargeIn(TRUNK_STRUCT *pOneTrunk)
{

	Acs_Voc_Bargein_Config bargein_config;

	 bargein_config.enable = cfg_bargeInEnable;
	  bargein_config.DTMFSignalCutOff = cfg_bargeDTMFCutOff;
	  bargein_config.maxPersonSilenceTime = cfg_bargePersonSilenceTime;
	  bargein_config.silenceTimeout = cfg_bargeSilenceTout;
	  bargein_config.maxMusicVocTime = cfg_bargeMusicOnTime;
	  bargein_config.voiceTimeout = cfg_bargeVocTout;
	  bargein_config.maxToneSilenceTime = cfg_bargeToneSilenceTime;
	  bargein_config.getNormalEvent=cfg_bargeGetNormEvt;

	int  ret = XMS_ctsSetParam( g_acsHandle, &pOneTrunk->VocDevID, VOC_PARAM_BARGEIN, sizeof(Acs_Voc_Bargein_Config), (void *)&bargein_config);
...
DJ_S32 stopBargeIn(TRUNK_STRUCT *pOneTrunk)
{
	Acs_Voc_Bargein_Config bargein_config;

	  bargein_config.enable = 0;
	  bargein_config.DTMFSignalCutOff = 1;
	  bargein_config.maxPersonSilenceTime = 700;
	  bargein_config.silenceTimeout = 12000;
	  bargein_config.maxMusicVocTime = 800;
	  bargein_config.voiceTimeout = 11000;
	  bargein_config.maxToneSilenceTime = 200;
...
//void  stopAmd(int ch)
void  stopAmd( TRUNK_STRUCT *pOneTrunk)
{
	int ch;
...
//add by zcq at 20120828
DJ_S32  My_AMDdetect( TRUNK_STRUCT *pOneTrunk, Acs_Evt_t *pAcsEvt )
{
	...
	if(pAcsEvt->m_s32EventType == XMS_EVT_RECVIODATA)
	{
		...
		{
			if(pIOData->m_u16IoType == XMS_IO_TYPE_BARGEIN)
			{
				Acs_Voc_Bargein_Event *pEvent = (Acs_Voc_Bargein_Event*)(FetchIOData(pAcsEvt));

				//printf event!!
#if 0
				sprintf ( TmpStr, "baregIn Evnet:0x%x", pEvent->event);

				WriteMsgLog(TmpStr);

#endif
				
				result=doAMDdetect(pEvent,pOneTrunk->VocDevID.m_s16ChannelID);
	...
	case TRK_SIM_CALLOUT:
		...
		if ( pAcsEvt->m_s32EventType == XMS_EVT_CALLOUT )	
		{
			...
			if(  pAcsEvt->m_DeviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_TRUNK )
			{
				...
				sprintf ( TmpStr, "TRK_SIM_CALLOUT!!");
				WriteMsgLog(TmpStr);
...
//					DrawMain_VocInfo ( pOneTrunk );
	...
	case TRK_SIM_ANALOG_OFFHOOK:
		if ( pAcsEvt->m_s32EventType == XMS_EVT_DEV_TIMER )
		{
			...
			sprintf ( TmpStr, "Wait Dial Tone TimeOut!!\n");
			WriteMsgLog(TmpStr);
		...
		if ( TmpGtd != -1 )
		{
			...
			else if ( TmpGtd == 'G' )		// Dial Tone
			{
				sprintf ( TmpStr, "Dial Tone : %c", TmpGtd );
				...
				// start send DTMF
				 ...
				 sprintf ( TmpStr, "callNo. : %s", cfg_SimCalledNum );
				
				WriteMsgLog(TmpStr);

				 if( ret > 0 )
				 {
					 //record---zcq at 20120828

					   sprintf ( TmpStr, "E:\\rec\\%s.pcm", cfg_SimCalledNum );

						RecordFile(&pAcsEvt->m_DeviceID,TmpStr,8000L*60, false);
					...
					//delay
					XMS_ctsSetDevTimer ( g_acsHandle, &pOneTrunk->deviceID, strlen(cfg_SimCalledNum)*200+100 );	// 根据号码长度，进行延时
				
					Change_State(pOneTrunk,TRK_WAIT_DIAL);
	...
	case TRK_WAIT_DIAL:
		...
		if ( pAcsEvt->m_s32EventType == XMS_EVT_DEV_TIMER )
		{
				...
				int ret=startBargeIn(pOneTrunk);

				if((ret>0)&&(cfg_bargeInEnable==1))
				{
					  startAmd(pOneTrunk);
				}
				else
	...
	case TRK_SIM_ANALOG_DIALING:
		if ( pAcsEvt->m_s32EventType == XMS_EVT_DEV_TIMER )
		{
			...
			sprintf (TmpStr, "Wait Callee Offhook TimeOut");
				
			WriteMsgLog(TmpStr);

			XMS_ctsClearCall ( g_acsHandle, &pOneTrunk->deviceID, 0, NULL );
			
			//stop bargeIn and AMD func----zcq at 20120828

			stopBargeIn(pOneTrunk);
		...
		TmpGtd   = My_GetGtdOrPVDCode ( pAcsEvt );
		 ...
		 AMDresult = My_AMDdetect( pOneTrunk,pAcsEvt); //---zcq at 20120828

	
		
		if (( TmpGtd != -1 )||(AMDresult!=-1))
		{
			if ( TmpGtd == 'I' || TmpGtd == 'J' || TmpGtd == 'K' )		// Busy Tone
			{	
			...
			else if(TmpGtd == 'H')
			{
				...
				sprintf ( TmpStr, "Callee eco_ring tone : %c", TmpGtd );

				WriteMsgLog(TmpStr);
			}
		//	else if(TmpGtd == 'h' || TmpGtd == 'b' ) 
			else if((TmpGtd == 'h')||(AMDresult==AMD_RESULT_VIRTUAL_PERSON))
			{
				...
				else if(AMDresult==AMD_RESULT_VIRTUAL_PERSON)
				{
					sprintf ( TmpStr, "AMD_RESULT_VIRTUAL_PERSON!!");
			...
			//	PlayDTMF ( &pOneTrunk->VocDevID, "12345" );		// play dtmf
				...
				XMS_ctsSetDevTimer ( g_acsHandle, &pOneTrunk->deviceID, 2000);	// start a timer: 2 Seconds


				Change_State ( pOneTrunk, TRK_PLAYDTMF );
			}
			else if((AMDresult==AMD_RESULT_MUSIC)||(AMDresult==AMD_RESULT_PROMPT_TONE))				
			{//记录检测结果－－－zcq at 20120828

				if(AMDresult==AMD_RESULT_MUSIC)
				{
					sprintf ( TmpStr, "AMD_RESULT_MUSIC!!");

					WriteMsgLog(TmpStr);
				}
				else if(AMDresult==AMD_RESULT_PROMPT_TONE)
				{ //提示音，则退出检测
					...
					//stop bargeIn and AMD func----zcq at 20120828
						...
						sprintf ( TmpStr, "AMD_RESULT_PROMPT_TONE!!");
	...
	case TRK_PLAYDTMF:
	    ...
	    // if ( CheckPlayGTGEnd ( pOneTrunk, pAcsEvt) )	/*play Over Event*/
		{
			// Clear Call
			XMS_ctsClearCall ( g_acsHandle, &pOneTrunk->deviceID, 0, NULL );			
