...
BOOL CXMSApi_RecordSet::OnInitDialog() 
{
	...
	if ( g_RecordProperty.m_u8SRCMode == XMS_SRC_11K )
	...
	sprintf ( TmpStr, "%c", g_RecordProperty.m_u8StopCode );
	...
	sprintf ( TmpStr, "%d", g_RecordProperty.m_u8TaskID );
	GetDlgItem ( IDC_EDIT_RECORD_TASKID )->SetWindowText (TmpStr);
...
void CXMSApi_RecordSet::OnOK() 
{
	...
	if ( ((CButton *)GetDlgItem ( IDC_RADIO_SRC_11K ))->GetCheck() )
		g_RecordProperty.m_u8SRCMode = XMS_SRC_11K;
	// g_RecordProperty . m_u8StopMode
	...
	g_RecordProperty.m_u8StopCode = TmpStr[0];
