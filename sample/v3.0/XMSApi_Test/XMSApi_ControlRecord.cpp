...
BOOL CXMSApi_ControlRecord::OnInitDialog() 
{
	...
	if ( g_ControlRecord.m_u32DelayStop == XMS_REC_DELAY_STOP_DISABLE )
	{
		((CButton*)GetDlgItem(IDC_RADIO_XMS_REC_DELAY_STOP_DISABLE))->SetCheck(true);
	}
	else
	{
		((CButton*)GetDlgItem(IDC_RADIO_XMS_REC_DELAY_STOP_ENABLE))->SetCheck(true);
	}
...
void CXMSApi_ControlRecord::OnOK() 
{
	...
	if ( TRUE == ((CButton*)GetDlgItem(IDC_RADIO_XMS_REC_DELAY_STOP_DISABLE))->GetCheck() )
	{
		g_ControlRecord.m_u32DelayStop = XMS_REC_DELAY_STOP_DISABLE;
	}
	else
	{
		g_ControlRecord.m_u32DelayStop = XMS_REC_DELAY_STOP_ENABLE;
	}
