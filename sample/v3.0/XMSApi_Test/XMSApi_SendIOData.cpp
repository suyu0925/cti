...
TYPE_MY_SEND_IO_DATA IoDataStru[MAX_IO_TYPE_NUM] =
{
	...
	{XMS_IO_TYPE_MSK,			"IO_TYPE_MSK"},
	{XMS_IO_TYPE_VSD,			"IO_TYPE_VSD"},
};
...
void CXMSApi_SendIOData::OnButton1() 
{
	...
	static char				MsgStr[2048];

	memset(TmpStr, 0, 2048);
	memset(MsgStr, 0, 2048);
	...
	GetDlgItem ( IDC_EDIT_DataContent )->GetWindowText ( TmpStr, 2048 );
	...
	//	case 8://IO_TYPE_DCH_I_FRAME
		...
		case 10://IO_TYPE_DCH_DEBUG
		{
			...
			if ( r < 0 )
			{
				sprintf ( MsgStr, "X(%d) XMS_ctsSendIOData( %s, %d, [%c-%c] ) FAIL! (%s)", 
					r, IoDataStru[iIndex].TypeStr, iLen, TmpStr[0],TmpStr[iLen-1], 
					GetString_DeviceAll (&DevOpened[iOpenedIndex1].DevID)  );
				AddMsg ( MSG_TYPE_FUNCTION, MsgStr );
			}
			else
			{
				sprintf ( MsgStr, "XMS_ctsSendIOData( %s, %d, [%02x-%02x] ) OK! (%s)", 
					IoDataStru[iIndex].TypeStr, iLen, TmpStr[0],TmpStr[iLen-1],  
					GetString_DeviceAll (&DevOpened[iOpenedIndex1].DevID)  );
				AddMsg ( MSG_TYPE_FUNCTION, MsgStr );
