#ifndef _XMS_CONFDEMO_EVENT_H_
#define _XMS_CONDEMO_EVENT_H_
...
#define		M_OneConf(DevID)		AllDeviceRes[(DevID).m_s8ModuleID].pConf[(DevID).m_s16ChannelID]
...
typedef	struct
{
	...
	long	lConfNum;			// the XMS_DEVMAIN_CONFERENCE number in this DSP
	long	lConfOpened;		// the Conf number opened by OpenDevice()
	long	lConfFreeNum;		// the free Conf number in this DSP
	CONF_STRUCT	*pConf;			// the structer of Conf, alloc as need
} TYPE_XMS_DSP_DEVICE_RES_CONFDEMO;
