...
void	OpenTrunkDevice ( TRUNK_STRUCT *pOneTrunk )
{
	...
	{
		if ( XMS_DEVSUB_UNUSABLE == pOneTrunk->deviceID.m_s16DeviceSub 
		  || XMS_DEVSUB_ANALOG_EMPTY == pOneTrunk->deviceID.m_s16DeviceSub
		  || XMS_DEVSUB_BASE == pOneTrunk->deviceID.m_s16DeviceSub )
		{
			return;
		}
...
void	OpenDeviceOK ( DeviceID_t *pDevice )
{
	...
	AllDeviceRes[pDevice->m_s8ModuleID].bErrFlag = false;
	...
	if ( pDevice->m_s16DeviceMain == XMS_DEVMAIN_INTERFACE_CH )
	{
		...
		if ( pOneTrunk->deviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_USER )
		{
			...
			// modify the count
			g_iTotalUserOpened ++;
			AllDeviceRes[pDevice->m_s8ModuleID].iAnalogUserOpened++;
...
void	CloseTrunkDevice ( TRUNK_STRUCT *pOneTrunk )
{
	...
	{
		char str[200];
		sprintf(str, "Close Device(DSP: %d, CH: %d, MT: %d, SFT: %d, CallID[FlowType: %d, FlowChID: %d]) Failed(Ret: %d)", 
			pOneTrunk->deviceID.m_s8ModuleID, 
			pOneTrunk->deviceID.m_s16ChannelID, 
			pOneTrunk->deviceID.m_s16DeviceMain, 
			pOneTrunk->deviceID.m_s16DeviceSub, 
			pOneTrunk->deviceID.m_CallID.m_s32FlowType,
			pOneTrunk->deviceID.m_CallID.m_s32FlowChannel,
			r );
		AddMsg ( str );
	}
}
...
void	CloseVoiceDevice ( VOICE_STRUCT *pOneVoice )
{
	...
	if ( r < 0 )
	{
		...
		sprintf(str, "Close Device(DSP: %d, CH: %d, MT: %d, SFT: %d, CallID[FlowType: %d, FlowChID: %d]) Failed(Ret: %d)", 
			...
			pOneVoice->deviceID.m_s16DeviceMain, 
			pOneVoice->deviceID.m_s16DeviceSub, 
			pOneVoice->deviceID.m_CallID.m_s32FlowType,
			pOneVoice->deviceID.m_CallID.m_s32FlowChannel,
			r );
		AddMsg ( str );
	}
}
...
void	ClosePcmDevice ( PCM_STRUCT *pOnePcm )
{
	...
	if ( r < 0 )
	{
		char str[200];
		sprintf(str, "Close Device(DSP: %d, CH: %d, MT: %d, SFT: %d, CallID[FlowType: %d, FlowChID: %d]) Failed(Ret: %d)", 
			pOnePcm->deviceID.m_s8ModuleID, 
			pOnePcm->deviceID.m_s16ChannelID, 
			pOnePcm->deviceID.m_s16DeviceMain, 
			pOnePcm->deviceID.m_s16DeviceSub, 
			pOnePcm->deviceID.m_CallID.m_s32FlowType,
			pOnePcm->deviceID.m_CallID.m_s32FlowChannel,
			r );
		AddMsg ( str );
	}
}
...
void	CloseBoardDevice ( DeviceID_t *pBoardDevID )
{
	...
	if ( r < 0 )
	{
		char str[200];
		sprintf(str, "Close Device(DSP: %d, CH: %d, MT: %d, SFT: %d, CallID[FlowType: %d, FlowChID: %d]) Failed(Ret: %d)", 
			pBoardDevID->m_s8ModuleID, 
			pBoardDevID->m_s16ChannelID, 
			pBoardDevID->m_s16DeviceMain, 
			pBoardDevID->m_s16DeviceSub, 
			pBoardDevID->m_CallID.m_s32FlowType,
			pBoardDevID->m_CallID.m_s32FlowChannel,
			r );
		AddMsg ( str );
	}
}
