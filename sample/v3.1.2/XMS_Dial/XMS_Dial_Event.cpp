...
int DevCmp( DeviceID_t Dev1, DeviceID_t Dev2 )
{
	if ( Dev1.m_s16DeviceMain == Dev2.m_s16DeviceMain
	  && Dev1.m_s16DeviceSub == Dev2.m_s16DeviceSub
	  && Dev1.m_s8ModuleID == Dev2.m_s8ModuleID
	 // && Dev1.m_s8MachineID == Dev2.m_s8MachineID
	  && Dev1.m_s16ChannelID == Dev2.m_s16ChannelID )
	  //&& Dev1.m_CallID.m_s32FlowType == Dev2.m_CallID.m_s32FlowType
	  //&& Dev1.m_CallID.m_s32FlowChannel == Dev2.m_CallID.m_s32FlowChannel )
	{
		return 1;
	}
	else
...
void	AddDeviceRes_Trunk ( DJ_S8 s8DspModID, Acs_Dev_List_Head_t *pAcsDevList )
{
	...
	if ( (AllDeviceRes[s8DspModID].lTrunkNum == 0) && (s32Num > 0) )		// the resources new added
	{
		...
		if( !AllDeviceRes[s8DspModID].pTrunk )
		{
            ...
            AllDeviceRes[s8DspModID].iAnalogUserOpened = 0;
	...
	{
		if ( AllDeviceRes[s8DspModID].lTrunkNum == s32Num )
		{
			sprintf ( TmpStr, "Org List Trunk Num [%d]%d is equal to ResEvtNum: %d." , s8DspModID, AllDeviceRes[s8DspModID].lTrunkNum, s32Num );
			AddMsg ( TmpStr );
			...
			for ( i = 0; i < s32Num; i ++ )
			{
				if ( 1 == DevCmp( AllDeviceRes[s8DspModID].pTrunk[i].deviceID, pDev[i] ) )
				{
					continue;
				}
				else
				{
					sprintf ( TmpStr, "Dev(DSP: %d, CH: %d, MT: %d, SFT: %d,FT: %d, FCH: %d) NEW Dev(DSP: %d, CH: %d, MT: %d, SFT: %d, FT: %d, FCH: %d) will reopen." , 
						AllDeviceRes[s8DspModID].pTrunk[i].deviceID.m_s8ModuleID,
						AllDeviceRes[s8DspModID].pTrunk[i].deviceID.m_s16ChannelID,
						AllDeviceRes[s8DspModID].pTrunk[i].deviceID.m_s16DeviceMain, 
						AllDeviceRes[s8DspModID].pTrunk[i].deviceID.m_s16DeviceSub, 
						AllDeviceRes[s8DspModID].pTrunk[i].deviceID.m_CallID.m_s32FlowType, 
						AllDeviceRes[s8DspModID].pTrunk[i].deviceID.m_CallID.m_s32FlowChannel, 
						pDev[i].m_s8ModuleID,
						pDev[i].m_s16ChannelID,
						pDev[i].m_s16DeviceMain, 
						pDev[i].m_s16DeviceSub,
						pDev[i].m_CallID.m_s32FlowType,
						pDev[i].m_CallID.m_s32FlowChannel
						);
					AddMsg ( TmpStr );
...
void	AddDeviceRes_Voice ( DJ_S8 s8DspModID, Acs_Dev_List_Head_t *pAcsDevList )
{
	...
	{
		if ( AllDeviceRes[s8DspModID].lVocNum == s32Num )
		{
			sprintf ( TmpStr, "Org List Voc Num [%d]%d is equal to ResEvtNum: %d." , s8DspModID, AllDeviceRes[s8DspModID].lVocNum, s32Num );
			AddMsg ( TmpStr );
			return;
		}
	}
...
void	AddDeviceRes_Pcm ( DJ_S8 s8DspModID, Acs_Dev_List_Head_t *pAcsDevList )
{
	...
	{
		if ( AllDeviceRes[s8DspModID].lPcmNum == s32Num )
		{
			sprintf ( TmpStr, "Org List Pcm Num [%d]%d is equal to ResEvtNum: %d." , s8DspModID, AllDeviceRes[s8DspModID].lPcmNum, s32Num );
			AddMsg ( TmpStr );
			return;
		}
...
void	AddDeviceRes_Board ( DJ_S8 s8DspModID, Acs_Dev_List_Head_t *pAcsDevList )
{
	...
	else if ( (AllDeviceRes[s8DspModID].lFlag == 1) && (s32Num > 0) )		// delete this resource
	{
		//if ( AllDeviceRes[s8DspModID].lPcmNum == s32Num )
		{
			sprintf ( TmpStr, "Org List Board Num[%d] %d is equal to ResEvtNum: %d." , s8DspModID, AllDeviceRes[s8DspModID].lFlag, s32Num );
			AddMsg ( TmpStr );
			return;
		}
...
DJ_Void EvtHandler(DJ_Void *esrParam)
{
	...
	switch ( pAcsEvt->m_s32EventType )
	{
		...
		case XMS_EVT_QUERY_ONE_DSP_START:
			if ( ( cfg_iPartWork == 0 ) || (pAcsEvt->m_DeviceID.m_s8ModuleID == cfg_iPartWorkModuleID) )
			{
				if ( pAcsEvt->m_s32EventType == XMS_EVT_QUERY_ONE_DSP_START && 1 == AllDeviceRes[pAcsEvt->m_DeviceID.m_s8ModuleID].lFlag )
				{
					g_iTotalUserOpened = g_iTotalUserOpened - AllDeviceRes[pAcsEvt->m_DeviceID.m_s8ModuleID].iAnalogUserOpened;
					//AllDeviceRes[pAcsEvt->m_DeviceID.m_s8ModuleID].iAnalogUserOpened = 0;
					//CloseAllDevice_Dsp ( pAcsEvt->m_DeviceID.m_s8ModuleID );
					TRACE("(DSP: %d) XMS_EVT_QUERY_ONE_DSP_START. \n", pAcsEvt->m_DeviceID.m_s8ModuleID );
				}
			}			
			break;
		...
		case XMS_EVT_UNIFAILURE:
			{
				...
				strcat ( TmpStr, TmpS );
				TRACE("*** XMS_EVT_UNIFAILURE: %s \n", TmpStr);
			}
			break;
        case XMS_EVT_MODMONITOR:
			{
				TRACE("*** XMS_EVT_MODMONITOR:  \n");
			}
			break;
		...
		default:
			...
			{
				ChannelWork ( &M_OneTrunk(pAcsEvt->m_DeviceID), pAcsEvt );
			}
			else if ( pAcsEvt->m_DeviceID.m_s16DeviceMain == XMS_DEVMAIN_VOICE )
			{
				...
				{
					ChannelWork ( &M_OneTrunk(*pDevID), pAcsEvt );
				}
			}
			break;
	}
