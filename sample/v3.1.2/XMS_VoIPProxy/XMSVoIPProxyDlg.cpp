...
void CXMSVoIPProxyDlg::saveConfig()
{
	...
	for( i=0; iter!=g_lsRoute.end(); ++i, ++iter)
	{
		char buf[100] = "";
		CString str;
		str.Format("Route%d", i);
...
BOOL CXMSVoIPProxyDlg::OnInitDialog()
{
...
#ifdef _WIN64
	nRet = RegOpenKey(HKEY_LOCAL_MACHINE,"Software\\Wow6432Node\\DJXMS",&hKey);
#else
	...
	strcat(szBuf,"ini\\DJItpConfig.ini");
//		if(NULL != strstr(szBuf,"K200"))
//	int i=GetPrivateProfileInt((char *)"CONFIG_SERVER", (char *)"KeygoeMode", 0, (char *)szBuf);
	if(	GetPrivateProfileInt((char *)"CONFIG_SERVER", (char *)"KeygoeMode", 0, (char *)szBuf)==1 )
	{
		MAX_DEVICE_VOIP=64*MAX_DEVICE_BOARD;
	}

	if(NULL == g_deviceVoIP)
	{
		g_deviceVoIP=(XMS_VOIP_DEVICE *)malloc(sizeof(XMS_VOIP_DEVICE)*MAX_DEVICE_VOIP);
	}

	if(NULL == g_deviceRTP)
	{
		g_deviceRTP=(XMS_RTP_DEVICE*)malloc(sizeof(XMS_RTP_DEVICE)*MAX_DEVICE_VOIP);
	}
	// init user interface
	initUI();
