#include "stdafx.h"
#include "XMS_HizRecDemo.h"
#include "XMS_HizRecDemoDlg.h"
...
#include "XMS_HizRecDemo_Sub.H"
#include "XMS_HizRecDemo_Event.H"
#include "XMS_HizRecDemo_String.H"
...
char			cfg_IniShortName[] = "\\XMS_HizRecDemo.INI";
...
DJ_U8			g_u8UnitID    = 2;
DJ_U8           g_u8BoardType = 0;    //reference to BOARD_TYPE
...
void	ReadFromConfig(void)
{
	...
	g_u8BoardType = GetPrivateProfileInt ( "ConfigInfo", "BoardType", 0, cfg_IniName);
...
void	WriteToConfig(void)
{
	...
	sprintf ( TmpStr, "%d", g_u8BoardType);
	WritePrivateProfileString ( "ConfigInfo", "BoardType", TmpStr, cfg_IniName);
}
...
void DrawMain_CallInfo( TRUNK_STRUCT *pOneTrunk )
{
	...
	TRACE("****(DSP: %d, ch: %d) Calleecode: %s, CallerCode: %s \n", pOneTrunk->deviceID.m_s8ModuleID, pOneTrunk->deviceID.m_s16ChannelID, pOneTrunk->CalleeCode, pOneTrunk->CallerCode);
...
void	OpenDeviceOK ( DeviceID_t *pDevice )
{
	...
	if ( pDevice->m_s16DeviceMain == XMS_DEVMAIN_INTERFACE_CH )
	{
	    ...
	    if(pOneTrunk->deviceID.m_s16DeviceSub == XMS_DEVSUB_DIGITAL_REC)
		{
              ...
              sm.m_u8SwitchMode = XMS_DRS_SWITCH_AVAYA; //AVAYA
              //sm.m_u8EncodeType = XMS_Alaw_type; //A-Law
              sm.m_u8EncodeType = XMS_Ulaw_type; //A-Law
              sm.m_u8IsRecordSigData = 1;  //start to record the signal data from DSP
...
int		SearchOneRecordVoice (  TRUNK_STRUCT *pOneTrunk, DeviceID_t *pFreeVocDeviceID )
{
	...
	pOneVoice = &AllDeviceRes[s8ModID].pVoice[s16ChID];


	if ( (pOneVoice->State == VOC_FREE) 
	&& (pOneVoice->deviceID.m_s8ModuleID == s8ModID)
	&& (pOneVoice->deviceID.m_s16ChannelID == s16ChID))
	{
	...
	for ( i = 0; i < AllDeviceRes[s8SearchModID].lVocNum; i ++ )
	{
		...
		if ( (pOneVoice->deviceID.m_s8ModuleID == s8ModID)
	      && (pOneVoice->deviceID.m_s16ChannelID == s16ChID) )
		{
			*pFreeVocDeviceID = pOneVoice->deviceID;
...
void TrunkWork ( TRUNK_STRUCT *pOneTrunk, Acs_Evt_t *pAcsEvt )
{
	...
	if ( XMS_EVT_DRS_CALL_NUMS == pAcsEvt->m_s32EventType )
	{
	...
	case TRK_FREE:
		if ( pOneTrunk->deviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_REC )
		{
			...
			if ( p != NULL )
			{
				...
				ConvertRawFskToCallerID ( (unsigned char *)p, pIOData->m_u16IoDataLen,pOneTrunk->CallerCode, 20 );
			}	
		}

		pCallControl = (Acs_CallControl_Data *)FetchEventData(pAcsEvt);

		if ( pAcsEvt->m_s32EventType == XMS_EVT_CALLIN || pAcsEvt->m_s32EventType == XMS_EVT_CALLOUT)				// OffHook
		{
			// release the Voice for get FSK
			if ( pOneTrunk->deviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_REC && BOARD_TYPE_A == g_u8BoardType )
			{
				My_DualUnlink ( &pOneTrunk->VocDevID, &pOneTrunk->deviceID );
			...
			if ( (pOneTrunk->deviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_REC && BOARD_TYPE_D == g_u8BoardType )
				|| (pOneTrunk->deviceID.m_s16DeviceSub == XMS_DEVSUB_DIGITAL_REC ) )
			{
				if ( XMS_DRS_CALL_DIRECT_CALL_OUT  == pCallControl->m_u8DRSCallDirectFlag )
				{
					sprintf ( pOneTrunk->CalleeCode, pCallControl->m_s8CalledNum );
					sprintf ( pOneTrunk->CallerCode, pCallControl->m_s8CallingNum );
			...
			{
				// start record
				if ( SearchOneRecordVoice ( pOneTrunk,  &FreeVocDeviceID ) >= 0 )
				{
					pOneTrunk->VocDevID = FreeVocDeviceID;
					...
					sprintf ( FileName, "%s\\DRSRec_T%d_%d_V%d_%d.pcm", cfg_VocPath, pOneTrunk->deviceID.m_s8ModuleID, pOneTrunk->deviceID.m_s16ChannelID, FreeVocDeviceID.m_s8ModuleID,  FreeVocDeviceID.m_s16ChannelID);
			...
			TRACE("(DSP: %d, CH: %d)Call In(MT: 0X%X, SMT: 0X%X, EvtType: 0x%x)\n", pOneTrunk->deviceID.m_s8ModuleID, pOneTrunk->deviceID.m_s16ChannelID, pOneTrunk->deviceID.m_s16DeviceMain,pOneTrunk->deviceID.m_s16DeviceSub, pAcsEvt->m_s32EventType );
			// change state
			Change_State ( pOneTrunk, TRK_OFFHOOK );
		}
		break;

	case TRK_OFFHOOK:		// Wait OnHook
		pCallControl = (Acs_CallControl_Data *)FetchEventData(pAcsEvt);
	    ...
	    if ( pAcsEvt->m_s32EventType == DES_DIGITAL_REC_VAD_VOC )				// Detect voice on line
		{
			TRACE("*******User can start to record voice when receiving this EVT\n");
		}

	    if ( pAcsEvt->m_s32EventType == DES_DIGITAL_REC_VAD_NON_VOC )		    // Detect non-voice on line
		{
			TRACE("*******User can stop to record voice when receiving this EVT\n");
		}
