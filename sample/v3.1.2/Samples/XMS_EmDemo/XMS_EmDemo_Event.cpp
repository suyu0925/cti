...
DJ_Void EvtHandler(DJ_Void *esrParam)
{
	...
	switch ( pAcsEvt->m_s32EventType )
	{
		...
		default:
			...
			{
				if ( pAcsEvt->m_DeviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_EM )
				{
					//TRACE("*****(DSP: %d, CH: %d) Recv Evnt(0X%x). \n", pAcsEvt->m_DeviceID.m_s8ModuleID, pAcsEvt->m_DeviceID.m_s16ChannelID, pAcsEvt->m_s32EventType );
					EMWork( &M_OneTrunk(pAcsEvt->m_DeviceID), pAcsEvt );	
				}
				else if ( pAcsEvt->m_DeviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_VOC4W 
					   || pAcsEvt->m_DeviceID.m_s16DeviceSub == XMS_DEVSUB_ANALOG_VOC2W )
				{
					EMVocWork( &M_OneTrunk(pAcsEvt->m_DeviceID), pAcsEvt );						
				}
				else
				{
			...
			else if ( pAcsEvt->m_DeviceID.m_s16DeviceMain == XMS_DEVMAIN_VOICE )
			{
				...
				if ( pDevID->m_s16DeviceMain == XMS_DEVMAIN_INTERFACE_CH )
				{
					...
					if ( pDevID->m_s16DeviceSub == XMS_DEVSUB_ANALOG_EM )
					{
						EMWork( &M_OneTrunk(*pDevID), pAcsEvt );	
					}
					else if ( pDevID->m_s16DeviceSub == XMS_DEVSUB_ANALOG_VOC4W
						   || pDevID->m_s16DeviceSub == XMS_DEVSUB_ANALOG_VOC2W  )
					{
						EMVocWork( &M_OneTrunk(*pDevID), pAcsEvt );	
					}
					else
					{
						TrunkWork ( &M_OneTrunk(*pDevID), pAcsEvt );				
