...
// ----------------------------------------------------------------------------------------------------
TYPE_XMS_DSP_DEVICE_RES_FAXDEMO	AllDeviceRes[MAX_DSP_MODULE_NUMBER_OF_XMS];
...
void	FreeOneDeviceRes ( int ID )
{
	...
	memset ( &AllDeviceRes[ID], 0, sizeof (TYPE_XMS_DSP_DEVICE_RES_FAXDEMO) );
}
...
{
	int		i, j;
	int		TrkCount, VocCount, PcmCount, ModuleCount, FaxCount;
	int		iModSeqID;

	// Remember the AllDeviceRes's Interface,Voice,PCM, Fax channel
	ModuleCount = TrkCount = VocCount = PcmCount = FaxCount = 0;
	for ( i = 0; i < MAX_DSP_MODULE_NUMBER_OF_XMS; i ++ )
	{
...
void	AddDeviceRes_Fax ( DJ_S8 s8DspModID, Acs_Dev_List_Head_t *pAcsDevList )
{
	...
	if ( (AllDeviceRes[s8DspModID].lFaxNum == 0) && (s32Num > 0) )		// the resources new added
	{
	...
	else if ( (AllDeviceRes[s8DspModID].lFaxNum > 0) && (s32Num == 0) )		// delete this resource
	{
		// if some devices did not close, close them
		for ( i = 0; i < AllDeviceRes[s8DspModID].lFaxNum; i ++ )
		{
			if ( AllDeviceRes[s8DspModID].pFax[i].State != FAX_WAITOPEN )
				CloseDeviceOK ( &AllDeviceRes[s8DspModID].pFax[i].deviceID );
		}
